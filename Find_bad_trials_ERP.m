...
for sub = dosubs
    ...
    vbadTrials = [];
    ind = 1:size(dat.trial,2);
    ind = setdiff(ind,vbadTrials);
    
    chans = {'O1','PO7','PO3','O2','PO8','PO4'};
    choi = find(ismember(dat.label,chans));
    
    clear data
    
    data = nan(length(ind),size(dat.trial{1},1),size(dat.trial{1},2));
    
    count=0;
    for i = ind
        count = count+1;
        data(count,:,:) = dat.trial{i};
    end

    
    B = data(:,choi,:);
    
%     imagesc(squeeze(nanmean(B,2)));
%     colorbar
    
    Bmax = squeeze(max(max(B,[],2),[],3));
    Bmin = squeeze(min(min(B,[],2),[],3));
    ...
    save(fullfile(sessDir,'ERPvbadTrials.mat'),'vbadTrials');
