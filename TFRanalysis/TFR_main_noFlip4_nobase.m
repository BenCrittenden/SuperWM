...
%%
    ...
    conlc = squeeze(nanmean(nanmean(AllSubDat(:,:,right_ind,foi,toi),2),3));
    conli = squeeze(nanmean(nanmean(AllSubDat(:,:,left_ind,foi,toi),2),3));
    
    conrc = squeeze(nanmean(nanmean(AllSubDat(:,:,left_ind,foi,toi),2),3));
    conri = squeeze(nanmean(nanmean(AllSubDat(:,:,right_ind,foi,toi),2),3));
...
%     toi = find((tfdat.time*1000)>=trange(1)&(tfdat.time*1000)<=trange(2));
    ...
    h(1) = plot(times(toi), lineH,'LineWidth',3,...
        'Color',[1 0 0]);
    h(2) = plot(times(toi), lineL, 'LineWidth',3,...
        'Color',[0 0 1]);
    % h(3) = plot(times, lineA,'LineWidth',3,...
    %                          'Color',[.3 .3 .3]);
    
    shadedErrorBar(times(toi),lineH,H_error,'r',1);
    shadedErrorBar(times(toi),lineL,L_error,'b',1);
...
%     figure,
%     hold on
%     plot(times(toi),lat(High_WM,:)','r');
%     plot(times(toi),lat(Low_WM,:)','b');
%     hold off
