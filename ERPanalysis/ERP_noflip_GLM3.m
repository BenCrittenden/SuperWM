%%
%The purpose of this GLM is to see what explains the set-size differece the
%best: contra, ipsi or contra-ipsi.
...
for sub = dosubs
    ...
    for sess = 1:length(sub_details(sub).in_fname)
        ...
        right_ind = find(ismember(dat.label,right_chans));
            ...
            for tpt = 1:size(data,3)
                ...
                des_mat = horzcat(cl); %,il,ci);
                
                B_ss(tpt,:) = glmfit(des_mat,SS);
                B_ac(tpt,:) = glmfit(des_mat,Acc);
                
                clear cl il ci

            end            
                
        AllSessBeta_ss(sess,:,:) = B_ss;
    ...
    AllSubBeta_ss(sub,:,:) = squeeze(nanmean(AllSessBeta_ss,1));
...
AllSubBeta = AllSubBeta_ss;

% Hbeta_c = smooth(squeeze(nanmean(AllSubBeta(High_WM,toi,2),1)),smf);
% Hbeta_i = smooth(squeeze(nanmean(AllSubBeta(High_WM,toi,3),1)),smf);
Hbeta_ci = smooth(squeeze(nanmean(AllSubBeta(High_WM,toi,2),1)),smf);

% Lbeta_c = smooth(squeeze(nanmean(AllSubBeta(Low_WM,toi,2),1)),smf);
% Lbeta_i = smooth(squeeze(nanmean(AllSubBeta(Low_WM,toi,3),1)),smf);
Lbeta_ci = smooth(squeeze(nanmean(AllSubBeta(Low_WM,toi,2),1)),smf);
...
% plot(times, Hbeta_c,'-',  'LineWidth',3,...
%                          'Color',[1 0 0]);
% plot(times, Hbeta_i,'-',  'LineWidth',3,...
%                          'Color',[0 1 0]);
plot(times, Hbeta_ci,'-', 'LineWidth',3,...
                         'Color',[0 0 1]);

% plot(times, Lbeta_c,'--',  'LineWidth',3,...
%                          'Color',[1 0 0]);
% plot(times, Lbeta_i,'--',  'LineWidth',3,...
%                          'Color',[0 1 0]);
