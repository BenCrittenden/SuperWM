...
Hconlc = squeeze(nanmean(AllSubDat(High_WM,:,right_ind,toi),3));
Hconli = squeeze(nanmean(AllSubDat(High_WM,:,left_ind,toi),3));

Hconrc = squeeze(nanmean(AllSubDat(High_WM,:,right_ind,toi),3));
Hconri = squeeze(nanmean(AllSubDat(High_WM,:,left_ind,toi),3));

Lconlc = squeeze(nanmean(AllSubDat(Low_WM,:,right_ind,toi),3));
Lconli = squeeze(nanmean(AllSubDat(Low_WM,:,left_ind,toi),3));

Lconrc = squeeze(nanmean(AllSubDat(Low_WM,:,right_ind,toi),3));
Lconri = squeeze(nanmean(AllSubDat(Low_WM,:,left_ind,toi),3));
...
lineH = smooth(squeeze(nanmean(Hlat,1)),smf);
lineL = smooth(squeeze(nanmean(Llat,1)),smf);

H_error = smooth(Hlat_error,smf);
L_error = smooth(Llat_error,smf);
...
%     for r=1:size(Hlat,1)
%         x(r,:) = smooth(Hlat(r,:),smf);
%     end
%     
%     for r=1:size(Llat,1)
%         y(r,:) = smooth(Llat(r,:),smf);
%     end
% 
%     Htest = x(:,tp);
%     Ltest = y(:,tp);
    ...
    [H(tp) pval(tp)] = ttest2(Htest,Ltest);
