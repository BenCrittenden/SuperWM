            ...
            for f = 1:length(freqs)
                ...
                for tpt = 1:size(data,4)
                    ...
                    for side = [1 2]
                        ...
                        if ~isnan(toFit(1))
                            B(chn,f,tpt,side,:) = glmfit(des_matTemp,toFit);
                        else
                            B(chn,f,tpt,side,:) = [NaN NaN];% NaN];
                        end
...
betaLc = smooth(squeeze(nanmean(nanmean(nanmean(AllSubBeta(dosubs,Lind,:,toi,1,regr),1),2),3)),smf);
betaLi = smooth(squeeze(nanmean(nanmean(nanmean(AllSubBeta(dosubs,Rind,:,toi,1,regr),1),2),3)),smf);

betaRc = smooth(squeeze(nanmean(nanmean(nanmean(AllSubBeta(dosubs,Rind,:,toi,2,regr),1),2),3)),smf);
betaRi = smooth(squeeze(nanmean(nanmean(nanmean(AllSubBeta(dosubs,Lind,:,toi,2,regr),1),2),3)),smf);
